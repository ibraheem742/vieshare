...
export function ProductCard({
  ...
  if (product.images && product.images.length > 0) {
    console.log(`ProductCard - "${product.name}" with images:`, {
      name: product.name,
...
{imageUrl ? ( // Use imageUrl here
              <Image
                ...
                loading="lazy"
                onLoad={() => {
                  console.log(`✅ Image loaded successfully for "${product.name}":`, imageUrl); // <--- Change this line
                }}
                onError={(e) => {
                  console.error(`❌ Image failed to load for "${product.name}":`, imageUrl); // <--- Change this line
